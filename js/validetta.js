...
( function ( $ ) {
    ...
    var messages = {
        ...
        minChecked  : 'Please select an option.',
    ...
    Validetta.prototype.events = function(){
        ...
        if( this.options.realTime === true ){
            // handle change event for form elements (without checkbox/radio)
            $( this.form ).find( '[data-validetta]' ).not( '[type=radio]' ).on( 'change', function( e ){
                // field to be controlled transferred to global variable
                fields = $( this );
                return that.init.call( that, e );
            });
            // handle click event for checkboxes/radio
            $( this.form ).find( '[data-validetta][type=radio]' ).on( 'click', function( e ){
                // fields to be controlled transferred to global variable
                fields = that.form.querySelectorAll( '[data-validetta][type=radio][name='+ this.name +']' );
                return that.init.call( that, e );
            });
        }
        ...
        for ( var i = fields.length - 1; i >= 0; i-- ) {
            ...
            for ( var j = _methods.length - 1; j >= 0; j-- ) {
                ...
                if( _methods[j] === 'required' ){
                    ...
                    if( _elType === 'radio' && !that.check.radio.checked( _el ) ){ _errors += messages.radio+'<br />'; }
                ...
                if( reg.test( _methods[j] ) ){
                    ...
                    }else if( rules[0] === 'maxChecked' && !that.check.radio.maxChecked.call( that, _el, rules[1] ) ){
                        // Redirect to the first checkbox
                        // I want to see the error message on the first element of checkbox/radio group
                        _el = that.form.querySelectorAll( 'input[type=radio][data-validetta][name='+ _el.name +']' )[0];
                        _errors += messages.maxChecked.replace( '{count}', rules[1] )+'<br />';
                    }else if( rules[0] === 'minChecked' && !that.check.radio.minChecked.call( that, _el, rules[1] ) ){
                        // Redirect to the first checkbox/radio
                        _el = that.form.querySelectorAll( 'input[type=radio][data-validetta][name='+ _el.name +']' )[0];
    ...
    Validetta.prototype.reset = function( _inp ){
        ...
        // and these elements are not checkbox/radio
        if( typeof _inp === 'undefined' || ( _inp.length > 1 && _inp[0].getAttribute('type') !== 'radio' ) ){ 
